...
const X_CODEX_TURN_STATE_HEADER: &str = "x-dcode-turn-state";
...
async fn connect_websocket(
    ...
    // If a Dcode-specific CA bundle is configured, build an explicit rustls connector so this
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "dcode.rate_limits" {
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-dcode-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-dcode-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
