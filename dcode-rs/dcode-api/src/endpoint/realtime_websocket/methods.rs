...
use dcode_client::maybe_build_rustls_client_config_with_custom_ca;
use dcode_utils_rustls_provider::ensure_rustls_crypto_provider;
use futures::SinkExt;
...
impl RealtimeWebsocketClient {
    ...
    pub async fn connect(
        ...
        // Realtime websocket TLS should honor the same custom-CA env vars as the rest of Dcode's
...
mod tests {
    ...
    use crate::endpoint::realtime_websocket::protocol::RealtimeTranscriptDelta;
    use crate::endpoint::realtime_websocket::protocol::RealtimeTranscriptEntry;
    use dcode_protocol::protocol::RealtimeInputAudioSpeechStarted;
    use dcode_protocol::protocol::RealtimeResponseCancelled;
    use http::HeaderValue;
    ...
    #[tokio::test]
    async fn realtime_v2_session_update_includes_dcode_tool_and_handoff_output_item() {
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
        let addr = listener.local_addr().expect("local addr");
