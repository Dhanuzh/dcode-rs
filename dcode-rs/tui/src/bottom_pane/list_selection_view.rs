...
mod tests {
    ...
    fn theme_picker_subtitle_uses_fallback_text_in_94x35_terminal() {
        ...
        let dcode_home = home.join(".dcode");
        let params =
            crate::theme_picker::build_theme_picker_params(None, Some(&dcode_home), Some(94));
    ...
    fn width_changes_do_not_hide_rows() {
        ...
        let items = vec![
            SelectionItem {
                name: "gpt-5.1-dcode".to_string(),
                description: Some(
                    "Optimized for Dcode. Balance of reasoning quality and coding ability."
                        .to_string(),
                ),
                is_current: true,
                dismiss_on_select: true,
            ...
            SelectionItem {
                name: "gpt-5.1-dcode-mini".to_string(),
                description: Some(
                    "Optimized for Dcode. Cheaper, faster, but less capable.".to_string(),
                ),
                dismiss_on_select: true,
                ..Default::default()
            },
            SelectionItem {
                name: "gpt-4.1-dcode".to_string(),
                description: Some(
