...
use dcode_app_server_protocol::AppSummary;
use dcode_app_server_protocol::ConfigLayerSource;
use dcode_backend_client::Client as BackendClient;
use dcode_chatgpt::connectors;
use dcode_core::ModelProviderInfo;
use dcode_core::config::Config;
use dcode_core::config::Constrained;
use dcode_core::config::ConstraintResult;
use dcode_core::config::types::ApprovalsReviewer;
use dcode_core::config::types::Notifications;
use dcode_core::config::types::WindowsSandboxModeToml;
use dcode_core::config_loader::ConfigLayerStackOrdering;
use dcode_core::find_thread_name_by_id;
use dcode_core::git_info::current_branch_name;
use dcode_core::git_info::get_git_repo_root;
use dcode_core::git_info::local_git_branches;
use dcode_core::mcp::McpManager;
use dcode_core::models_manager::manager::ModelsManager;
use dcode_core::plugins::PluginsManager;
use dcode_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use dcode_core::skills::model::SkillMetadata;
#[cfg(target_os = "windows")]
use dcode_core::windows_sandbox::WindowsSandboxLevelExt;
use dcode_features::FEATURES;
use dcode_features::Feature;
use dcode_otel::RuntimeMetricsSummary;
use dcode_otel::SessionTelemetry;
use dcode_protocol::ThreadId;
use dcode_protocol::account::PlanType;
use dcode_protocol::approvals::ElicitationRequestEvent;
use dcode_protocol::config_types::CollaborationMode;
use dcode_protocol::config_types::CollaborationModeMask;
use dcode_protocol::config_types::ModeKind;
use dcode_protocol::config_types::Personality;
use dcode_protocol::config_types::ServiceTier;
use dcode_protocol::config_types::Settings;
#[cfg(target_os = "windows")]
use dcode_protocol::config_types::WindowsSandboxLevel;
use dcode_protocol::items::AgentMessageContent;
use dcode_protocol::items::AgentMessageItem;
use dcode_protocol::models::MessagePhase;
use dcode_protocol::models::local_image_label_text;
use dcode_protocol::parse_command::ParsedCommand;
use dcode_protocol::protocol::AgentMessageDeltaEvent;
use dcode_protocol::protocol::AgentMessageEvent;
use dcode_protocol::protocol::AgentReasoningDeltaEvent;
use dcode_protocol::protocol::AgentReasoningEvent;
use dcode_protocol::protocol::AgentReasoningRawContentDeltaEvent;
use dcode_protocol::protocol::AgentReasoningRawContentEvent;
use dcode_protocol::protocol::ApplyPatchApprovalRequestEvent;
use dcode_protocol::protocol::BackgroundEventEvent;
use dcode_protocol::protocol::DcodeErrorInfo;
use dcode_protocol::protocol::CollabAgentSpawnBeginEvent;
use dcode_protocol::protocol::CreditsSnapshot;
use dcode_protocol::protocol::DeprecationNoticeEvent;
use dcode_protocol::protocol::ErrorEvent;
use dcode_protocol::protocol::Event;
use dcode_protocol::protocol::EventMsg;
use dcode_protocol::protocol::ExecApprovalRequestEvent;
use dcode_protocol::protocol::ExecCommandBeginEvent;
use dcode_protocol::protocol::ExecCommandEndEvent;
use dcode_protocol::protocol::ExecCommandOutputDeltaEvent;
use dcode_protocol::protocol::ExecCommandSource;
use dcode_protocol::protocol::ExitedReviewModeEvent;
use dcode_protocol::protocol::GuardianAssessmentEvent;
use dcode_protocol::protocol::GuardianAssessmentStatus;
use dcode_protocol::protocol::ImageGenerationBeginEvent;
use dcode_protocol::protocol::ImageGenerationEndEvent;
use dcode_protocol::protocol::ListCustomPromptsResponseEvent;
use dcode_protocol::protocol::ListSkillsResponseEvent;
use dcode_protocol::protocol::McpListToolsResponseEvent;
use dcode_protocol::protocol::McpStartupCompleteEvent;
use dcode_protocol::protocol::McpStartupStatus;
use dcode_protocol::protocol::McpStartupUpdateEvent;
use dcode_protocol::protocol::McpToolCallBeginEvent;
use dcode_protocol::protocol::McpToolCallEndEvent;
use dcode_protocol::protocol::Op;
use dcode_protocol::protocol::PatchApplyBeginEvent;
use dcode_protocol::protocol::RateLimitSnapshot;
use dcode_protocol::protocol::ReviewRequest;
use dcode_protocol::protocol::ReviewTarget;
use dcode_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
use dcode_protocol::protocol::StreamErrorEvent;
use dcode_protocol::protocol::TerminalInteractionEvent;
use dcode_protocol::protocol::TokenUsage;
use dcode_protocol::protocol::TokenUsageInfo;
use dcode_protocol::protocol::TurnAbortReason;
use dcode_protocol::protocol::TurnCompleteEvent;
use dcode_protocol::protocol::TurnDiffEvent;
use dcode_protocol::protocol::UndoCompletedEvent;
use dcode_protocol::protocol::UndoStartedEvent;
use dcode_protocol::protocol::UserMessageEvent;
use dcode_protocol::protocol::ViewImageToolCallEvent;
use dcode_protocol::protocol::WarningEvent;
use dcode_protocol::protocol::WebSearchBeginEvent;
use dcode_protocol::protocol::WebSearchEndEvent;
use dcode_protocol::request_permissions::RequestPermissionsEvent;
use dcode_protocol::request_user_input::RequestUserInputEvent;
use dcode_protocol::user_input::TextElement;
use dcode_protocol::user_input::UserInput;
use dcode_terminal_detection::Multiplexer;
use dcode_terminal_detection::TerminalInfo;
use dcode_terminal_detection::TerminalName;
use dcode_terminal_detection::terminal_info;
use dcode_utils_sleep_inhibitor::SleepInhibitor;
...
use dcode_core::DcodeAuth;
use dcode_core::ThreadManager;
use dcode_file_search::FileMatch;
use dcode_protocol::openai_models::InputModality;
use dcode_protocol::openai_models::ModelPreset;
use dcode_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use dcode_protocol::plan_tool::StepStatus;
use dcode_protocol::plan_tool::UpdatePlanArgs;
use dcode_protocol::protocol::AskForApproval;
use dcode_protocol::protocol::SandboxPolicy;
use dcode_utils_approval_presets::ApprovalPreset;
use dcode_utils_approval_presets::builtin_approval_presets;
...
const NUDGE_MODEL_SLUG: &str = "gpt-5.1-dcode-mini";
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: dcode_feedback::DcodeFeedback,
...
fn rate_limit_error_kind(info: &DcodeErrorInfo) -> Option<RateLimitErrorKind> {
    match info {
        DcodeErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        DcodeErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        DcodeErrorInfo::ResponseTooManyFailedAttempts {
...
/// and by sending requests back to dcode-core.
///
...
pub(crate) struct ChatWidget {
    ...
    dcode_op_tx: UnboundedSender<Op>,
    ...
    // Latest completed user-visible Dcode output that `/copy` should place on the clipboard.
    ...
    /// Tracks whether dcode-core currently considers an agent turn to be in progress.
    ///
    ...
    feedback: dcode_feedback::DcodeFeedback,
    ...
    session_network_proxy: Option<dcode_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: dcode_protocol::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask dcode-core to enumerate custom prompts for this session.
    ...
    fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
        ...
        let dcode_home = self.config.dcode_home.clone();
        tokio::spawn(async move {
            ...
            match find_thread_name_by_id(&dcode_home, &forked_from_id).await {
    ...
    fn on_thread_name_updated(&mut self, event: dcode_protocol::protocol::ThreadNameUpdatedEvent) {
    ...
    fn show_feedback_note(
        ...
        category: crate::app_event::FeedbackCategory,
        include_logs: bool,
        snapshot: dcode_feedback::FeedbackSnapshot,
    ) {
    ...
    fn handle_steer_rejected_error(&mut self, dcode_error_info: &DcodeErrorInfo) -> bool {
        matches!(
            dcode_error_info,
            DcodeErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            let limit_id = snapshot
                .limit_id
                .clone()
                .unwrap_or_else(|| "dcode".to_string());
            ...
            let is_dcode_limit = limit_id.eq_ignore_ascii_case("dcode");
            let warnings = if is_dcode_limit {
    ...
    fn on_mcp_startup_complete(&mut self, ev: McpStartupCompleteEvent) {
        let dcode_apps_ready = ev.ready.iter().any(|server| server == "dcode_apps");
        ...
        self.update_task_running_state();
        self.maybe_send_next_queued_input();
        if self.connectors_enabled() && dcode_apps_ready {
    ...
    fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        if ev.status == GuardianAssessmentStatus::Approved {
            ...
            let cell = if let Some(command) = guardian_command(&action) {
                history_cell::new_approval_decision_cell(
                    command,
                    dcode_protocol::protocol::ReviewDecision::Approved,
        ...
        let cell = if let Some(command) = guardian_command(&action) {
            history_cell::new_approval_decision_cell(
                command,
                dcode_protocol::protocol::ReviewDecision::Denied,
                history_cell::ApprovalDecisionActor::Guardian,
            )
        } else {
            match tool {
    ...
    fn on_patch_apply_end(&mut self, event: dcode_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: dcode_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let dcode_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: dcode_protocol::protocol::HookStartedEvent) {
    ...
    fn on_hook_completed(&mut self, event: dcode_protocol::protocol::HookCompletedEvent) {
        ...
        for entry in event.run.entries {
            let prefix = match entry.kind {
                dcode_protocol::protocol::HookOutputEntryKind::Warning => "warning: ",
                dcode_protocol::protocol::HookOutputEntryKind::Stop => "stop: ",
                dcode_protocol::protocol::HookOutputEntryKind::Feedback => "feedback: ",
                dcode_protocol::protocol::HookOutputEntryKind::Context => "hook context: ",
                dcode_protocol::protocol::HookOutputEntryKind::Error => "error: ",
            };
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: dcode_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let dcode_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
        ...
        widget.bottom_pane.set_windows_degraded_sandbox_active(
            dcode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                && matches!(
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<dcode_core::DcodeThread>,
        session_configured: dcode_protocol::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::DcodeOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("dcode.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "dcode.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
            ...
            SlashCommand::Logout => {
                if let Err(e) = dcode_core::auth::logout(
                    &self.config.dcode_home,
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::DcodeOp(Op::Undo));
            // }
            SlashCommand::Diff => {
            ...
            SlashCommand::Copy => {
                let Some(text) = self.last_copyable_output.as_deref() else {
                    self.add_info_message(
                        "`/copy` is unavailable before the first Dcode output or right after a rollback."
                            .to_string(),
                        /*hint*/ None,
                    );
                    return;
                ...
                match copy_result {
                    Ok(()) => {
                        ...
                        self.add_info_message(
                            "Copied latest Dcode output to clipboard.".to_string(),
                            hint,
                        );
                    }
                    Err(err) => {
            ...
            SlashCommand::TestApproval => {
                ...
                self.app_event_tx.send(AppEvent::DcodeEvent(Event {
                    id: "1".to_string(),
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = dcode_core::util::normalize_thread_name(&prepared_args) else {
                ...
                self.request_redraw();
                self.app_event_tx
                    .send(AppEvent::DcodeOp(Op::SetThreadName { name }));
                self.bottom_pane.drain_pending_submission_state();
            }
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                let Some(name) = dcode_core::util::normalize_thread_name(&name) else {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
                ...
                tx.send(AppEvent::DcodeOp(Op::SetThreadName { name }));
            }),
        );
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = dcode_core::connectors::connector_mention_slug(&app);
    ...
    pub(crate) fn handle_dcode_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, /*replay_kind*/ None);
    }

    pub(crate) fn handle_dcode_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_dcode_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                {
                } else if let Some(kind) = dcode_error_info.as_ref().and_then(rate_limit_error_kind)
                {
                    match kind {
            ...
            EventMsg::ItemCompleted(event) => {
                let item = event.item;
                if !from_replay && let dcode_protocol::items::TurnItem::UserMessage(item) = &item {
                ...
                if let dcode_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let dcode_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| dcode_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn open_theme_picker(&mut self) {
        let dcode_home = dcode_core::config::find_dcode_home().ok();
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.dcode_apps_ready;
    ...
    fn should_prefetch_rate_limits(&self) -> bool {
        ...
        self.auth_manager
            .auth_cached()
            .as_ref()
            .is_some_and(DcodeAuth::is_chatgpt_auth)
    }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::DcodeOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    /// Open a popup listing all configured providers. Selecting one persists
    /// it to config.toml as `model_provider` and asks the user to restart.
    pub(crate) fn open_provider_popup(&mut self) {
        ...
        providers.sort_by(|(a_id, _), (b_id, _)| {
            let a_current = *a_id == current_provider_id;
            let b_current = *b_id == current_provider_id;
            b_current.cmp(&a_current).then(a_id.cmp(b_id))
        });

        let app_event_tx = self.app_event_tx.clone();
        let items: Vec<crate::bottom_pane::SelectionItem> = providers
            .into_iter()
    ...
    pub(crate) fn open_realtime_audio_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Configure settings for Dcode.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            ..Default::default()
        });
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("dcode-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "dcode-auto-fast" => 0,
            "dcode-auto-balanced" => 1,
            "dcode-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-dcode")
            || preset.model.starts_with("gpt-5.1-dcode-max")
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let show_elevate_sandbox_hint = dcode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if dcode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && dcode_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.dcode_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match dcode_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.dcode_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.permissions.sandbox_policy.get(),
            Some(self.config.dcode_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        if !dcode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    ...
                    line!["Learn more: https://developers.openai.com/dcode/windows"],
                ])
        ...
        self.session_telemetry.counter(
            "dcode.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the Dcode agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/dcode/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "dcode.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "dcode.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "dcode.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(&mut self, preset: ApprovalPreset) {
        ...
        lines.push(line![
            "Learn more <https://developers.openai.com/dcode/windows>"
        ]);
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "dcode.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "dcode.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "dcode.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    fn set_service_tier_selection(&mut self, service_tier: Option<ServiceTier>) {
        ...
        self.app_event_tx
            .send(AppEvent::DcodeOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                approvals_reviewer: None,
                sandbox_policy: None,
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("dcode-auto-") {
    ...
    fn plugins_for_mentions(&self) -> Option<&[dcode_core::plugins::PluginCapabilitySummary]> {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = dcode_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("dcode resume {name}"));
    ...
    /// Forward an `Op` directly to dcode.
    pub(crate) fn submit_op(&mut self, op: Op) -> bool {
        ...
        if let Err(e) = self.dcode_op_tx.send(op) {
    ...
    fn refresh_plugin_mentions(&mut self) {
        ...
        let plugins = PluginsManager::new(self.config.dcode_home.clone())
    ...
    pub(crate) fn open_review_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx: &AppEventSender| {
                tx.send(AppEvent::DcodeOp(Op::Review {
                    review_request: ReviewRequest {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::DcodeOp(Op::Review {
                        review_request: ReviewRequest {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = dcode_core::git_info::recent_commits(cwd, /*limit*/ 100).await;
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[dcode_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
...
fn hook_event_label(event_name: dcode_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        dcode_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        dcode_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        dcode_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        dcode_protocol::protocol::HookEventName::Stop => "Stop",
    }
}

async fn fetch_rate_limits(base_url: String, auth: DcodeAuth) -> Vec<RateLimitSnapshot> {
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<dcode_core::git_info::CommitLogEntry>,
) {
