...
use dcode_protocol::plan_tool::PlanItemArg;
use dcode_protocol::plan_tool::StepStatus;
use dcode_protocol::plan_tool::UpdatePlanArgs;
use dcode_protocol::protocol::FileChange;
use dcode_protocol::protocol::McpAuthStatus;
use dcode_protocol::protocol::McpInvocation;
use dcode_protocol::protocol::SessionConfiguredEvent;
use dcode_protocol::request_user_input::RequestUserInputAnswer;
use dcode_protocol::request_user_input::RequestUserInputQuestion;
use dcode_protocol::user_input::TextElement;
use dcode_utils_cli::format_env_display::format_env_display;
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: dcode_protocol::protocol::ReviewDecision,
    actor: ApprovalDecisionActor,
) -> Box<dyn HistoryCell> {
    use dcode_protocol::protocol::NetworkPolicyRuleAction;
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<dcode_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<dcode_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(dcode_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`dcode-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.openai.com/dcode/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, dcode_protocol::mcp::Tool>,
    ...
    let mcp_manager = McpManager::new(Arc::new(PluginsManager::new(config.dcode_home.clone())));
...
mod tests {
    ...
    use dcode_core::config::types::McpServerConfig;
    use dcode_core::config::types::McpServerTransportConfig;
    use dcode_otel::RuntimeMetricTotals;
    use dcode_otel::RuntimeMetricsSummary;
    ...
    use dcode_protocol::mcp::CallToolResult;
    use dcode_protocol::mcp::Tool;
    use dcode_protocol::protocol::ExecCommandSource;
    use rmcp::model::Content;

    const SMALL_PNG_BASE64: &str = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==";
    async fn test_config() -> Config {
        let dcode_home = std::env::temp_dir();
        ConfigBuilder::default()
            .dcode_home(dcode_home.clone())
            .build()
            .await
            .expect("config")
    }
    ...
    fn session_configured_event(model: &str) -> SessionConfiguredEvent {
        SessionConfiguredEvent {
            ...
            approvals_reviewer: dcode_protocol::config_types::ApprovalsReviewer::User,
