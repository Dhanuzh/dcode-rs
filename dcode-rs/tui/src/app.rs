...
use dcode_ansi_escape::ansi_escape_line;
use dcode_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use dcode_app_server_client::InProcessAppServerClient;
use dcode_app_server_client::InProcessClientStartArgs;
use dcode_app_server_protocol::ClientRequest;
use dcode_app_server_protocol::ConfigLayerSource;
use dcode_app_server_protocol::ConfigWarningNotification;
use dcode_app_server_protocol::PluginInstallParams;
use dcode_app_server_protocol::PluginInstallResponse;
use dcode_app_server_protocol::PluginListParams;
use dcode_app_server_protocol::PluginListResponse;
use dcode_app_server_protocol::PluginReadParams;
use dcode_app_server_protocol::PluginReadResponse;
use dcode_app_server_protocol::PluginUninstallParams;
use dcode_app_server_protocol::PluginUninstallResponse;
...
use dcode_core::ForkSnapshot;
use dcode_core::ThreadManager;
use dcode_core::config::Config;
use dcode_core::config::ConfigBuilder;
use dcode_core::config::ConfigOverrides;
use dcode_core::config::edit::ConfigEdit;
use dcode_core::config::edit::ConfigEditsBuilder;
use dcode_core::config::types::ApprovalsReviewer;
use dcode_core::config::types::ModelAvailabilityNuxConfig;
use dcode_core::config_loader::CloudRequirementsLoader;
use dcode_core::config_loader::ConfigLayerStackOrdering;
use dcode_core::config_loader::LoaderOverrides;
use dcode_core::models_manager::collaboration_mode_presets::CollaborationModesConfig;
use dcode_core::models_manager::manager::RefreshStrategy;
use dcode_core::models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use dcode_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
#[cfg(target_os = "windows")]
use dcode_core::windows_sandbox::WindowsSandboxLevelExt;
use dcode_features::Feature;
use dcode_otel::SessionTelemetry;
use dcode_otel::TelemetryAuthMode;
...
use dcode_protocol::items::TurnItem;
use dcode_protocol::openai_models::ModelAvailabilityNux;
use dcode_protocol::openai_models::ModelPreset;
use dcode_protocol::openai_models::ModelUpgrade;
...
use dcode_protocol::protocol::FinalOutput;
...
use dcode_protocol::protocol::SessionConfiguredEvent;
use dcode_protocol::protocol::SessionSource;
use dcode_protocol::protocol::SkillErrorInfo;
use dcode_protocol::protocol::TokenUsage;
use dcode_terminal_detection::user_agent;
use dcode_utils_absolute_path::AbsolutePathBuf;
...
fn session_summary(
    ...
    let resume_command = dcode_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
async fn start_plugin_request_client(
    ...
    InProcessAppServerClient::start(InProcessClientStartArgs {
        ...
        enable_dcode_api_key_env: false,
        client_name: "dcode-tui".to_string(),
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_dcode_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_dcode_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
fn emit_system_bwrap_warning(app_event_tx: &AppEventSender) {
    let Some(message) = dcode_core::config::system_bwrap_warning() else {
        return;
    };
...
async fn emit_custom_prompt_deprecation_notice(app_event_tx: &AppEventSender, dcode_home: &Path) {
    let prompts_dir = dcode_home.join("prompts");
    let prompt_count = dcode_core::custom_prompts::discover_prompts_in(&prompts_dir)
        .await
        .len();
    if prompt_count == 0 {
        return;
...
fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_dcode_max_migration_prompt
            .unwrap_or(false),
...
async fn prepare_startup_tooltip_override(
    ...
    if let Err(err) = ConfigEditsBuilder::new(&config.dcode_home)
...
impl App {
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .dcode_home(self.config.dcode_home.clone())
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let mut builder = ConfigEditsBuilder::new(&self.config.dcode_home)
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let dcode_op_tx = if let Some(thread) = live_thread {
            crate::chatwidget::spawn_op_forwarder(thread)
        } else {
            let (tx, _rx) = unbounded_channel();
            tx
        };
        self.replace_chat_widget(ChatWidget::new_with_op_sender(init, dcode_op_tx));
    ...
    async fn drain_active_thread_events(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        loop {
            match rx.try_recv() {
                Ok(event) => self.handle_dcode_event_now(event),
    ...
    fn replay_thread_snapshot(
        ...
        if let Some(event) = snapshot.session_configured {
            self.handle_dcode_event_replay(event);
        }
        self.chat_widget
    ...
    pub async fn run(
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, &config.dcode_home).await;
        ...
        let feedback_audience = if auth_ref
            .and_then(DcodeAuth::get_account_email)
        ...
        let auth_mode = auth_ref
            .map(DcodeAuth::auth_mode)
            .map(TelemetryAuthMode::from);
        let session_telemetry = SessionTelemetry::new(
            ThreadId::new(),
            model.as_str(),
            model.as_str(),
            auth_ref.and_then(DcodeAuth::get_account_id),
            auth_ref.and_then(DcodeAuth::get_account_email),
            auth_mode,
            dcode_core::default_client::originator().value,
        ...
        {
            session_telemetry.counter("dcode.status_line", /*inc*/ 1, &[]);
        }
        ...
        #[cfg(target_os = "windows")]
        {
            let should_check = WindowsSandboxLevel::from_config(&app.config)
                ...
                && matches!(
                    ...
                    dcode_protocol::protocol::SandboxPolicy::WorkspaceWrite { .. }
                        | dcode_protocol::protocol::SandboxPolicy::ReadOnly { .. }
                )
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.dcode_home.clone();
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "dcode.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "dcode.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    if dcode_core::windows_sandbox::sandbox_setup_is_complete(dcode_home.as_path())
                    {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = dcode_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "dcode.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    dcode_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                session_telemetry.counter(
                                    dcode_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    /*inc*/ 1,
                                    &tags,
            ...
            AppEvent::BeginWindowsSandboxLegacySetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.chat_widget.show_windows_sandbox_setup_status();
                    tokio::task::spawn_blocking(move || {
                        if let Err(err) = dcode_core::windows_sandbox::run_legacy_setup_preflight(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                            ...
                            session_telemetry.counter(
                                "dcode.windows_sandbox.legacy_setup_preflight_failed",
                                /*inc*/ 1,
                                &[],
                            );
                            tracing::warn!(
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match dcode_core::windows_sandbox_read_grants::grant_read_root_non_elevated(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder::new(&self.config.dcode_home)
                    ...
                    match builder.apply().await {
                        Ok(()) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::DcodeOp(
            ...
            AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
                let builder = match kind {
                    RealtimeAudioDeviceKind::Microphone => {
                        ConfigEditsBuilder::new(&self.config.dcode_home)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.dcode_home.clone();
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                if let Err(err) = ConfigEditsBuilder::new(&self.config.dcode_home)
                    .with_profile(profile)
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let edit = dcode_core::config::edit::status_line_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.dcode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::TerminalTitleSetup { items } => {
                let ids = items.iter().map(ToString::to_string).collect::<Vec<_>>();
                let edit = dcode_core::config::edit::terminal_title_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.dcode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                let edit = dcode_core::config::edit::syntax_theme_edit(&name);
                let apply_result = ConfigEditsBuilder::new(&self.config.dcode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn handle_dcode_event_now(&mut self, event: Event) {
        ...
        self.chat_widget.handle_dcode_event(event);
    ...
    fn handle_dcode_event_replay(&mut self, event: Event) {
        self.chat_widget.handle_dcode_event_replay(event);
    }
    ...
    async fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
        ...
        self.handle_dcode_event_now(event);
        if self.backtrack_render_pending {
            tui.frame_requester().schedule_frame();
        }
        Ok(())
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("dcode-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> dcode_protocol::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
    ...
    fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.dcode_home))
        {
            crate::render::highlight::set_syntax_theme(theme);
            return;
        }
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: dcode_protocol::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = dcode_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    use dcode_protocol::protocol::SessionConfiguredEvent;
    use dcode_protocol::protocol::SessionSource;
    use dcode_protocol::protocol::ThreadRolledBackEvent;
    use dcode_protocol::protocol::TurnAbortReason;
    use dcode_protocol::protocol::TurnAbortedEvent;
    use dcode_protocol::protocol::TurnCompleteEvent;
    use dcode_protocol::protocol::TurnStartedEvent;
    ...
    async fn startup_custom_prompt_deprecation_notice_emits_when_prompts_exist() -> Result<()> {
        let dcode_home = tempdir()?;
        let prompts_dir = dcode_home.path().join("prompts");
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, dcode_home.path()).await;
    ...
    async fn startup_custom_prompt_deprecation_notice_skips_empty_prompts_dir() -> Result<()> {
        let dcode_home = tempdir()?;
        std::fs::create_dir_all(dcode_home.path().join("prompts"))?;
        let (tx_raw, mut rx) = unbounded_channel();
        let app_event_tx = AppEventSender::new(tx_raw);
    ...
    -> Result<()> {
        ...
        let approval_event = Event {
            ...
            msg: EventMsg::ExecApprovalRequest(
                dcode_protocol::protocol::ExecApprovalRequestEvent {
        ...
        app.handle_dcode_event_now(first_event);
        app.handle_dcode_event_now(second_event);
        app.chat_widget
    ...
    async fn replayed_turn_complete_submits_restored_queued_follow_up() {
        ...
        app.chat_widget
            .handle_dcode_event(session_configured.clone());
        app.chat_widget.handle_dcode_event(Event {
        ...
        let (chat_widget, _app_event_tx, _rx, mut new_op_rx) =
            make_chatwidget_manual_with_sender().await;
        app.chat_widget = chat_widget;
        app.chat_widget.handle_dcode_event(session_configured);
    ...
    async fn replace_chat_widget_keeps_replacement_terminal_title_cache_when_present() {
        ...
        replacement.last_terminal_title = Some("dcode".to_string());

        app.replace_chat_widget(replacement);

        assert_eq!(
            app.chat_widget.last_terminal_title,
            Some("dcode".to_string())
        );
    }
    ...
    async fn live_turn_started_refreshes_status_line_with_runtime_context_window() {
        ...
        app.handle_dcode_event_now(Event {
            id: "turn-started".to_string(),
            msg: EventMsg::TurnStarted(TurnStartedEvent {
                turn_id: "turn-1".to_string(),
    ...
    async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
        ...
        let dcode_home = tempdir()?;
        app.config.dcode_home = dcode_home.path().to_path_buf();
        ...
        let config = std::fs::read_to_string(dcode_home.path().join("config.toml"))?;
    ...
    -> Result<()> {
        ...
        let dcode_home = tempdir()?;
        app.config.dcode_home = dcode_home.path().to_path_buf();
        let config_toml_path = AbsolutePathBuf::try_from(dcode_home.path().join("config.toml"))?;
...
"#;
    ...
    async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
        ...
        app.chat_widget
            .set_service_tier(Some(dcode_protocol::config_types::ServiceTier::Fast));
    ...
    async fn make_test_app() -> App {
        ...
        let server = Arc::new(
            dcode_core::test_support::thread_manager_with_models_provider(
                DcodeAuth::from_api_key("Test API Key"),
                config.model_provider.clone(),
            ),
        );
        let auth_manager = dcode_core::test_support::auth_manager_from_auth(
            DcodeAuth::from_api_key("Test API Key"),
        );
        let file_search = FileSearchManager::new(config.cwd.clone(), app_event_tx.clone());
        let model = dcode_core::test_support::get_model_offline(config.model.as_deref());
        ...
        App {
            ...
            backtrack: BacktrackState::default(),
            backtrack_render_pending: false,
            feedback: dcode_feedback::DcodeFeedback::new(),
    ...
    fn test_session_telemetry(config: &Config, model: &str) -> SessionTelemetry {
        let model_info = dcode_core::test_support::construct_model_info_offline(model, config);
    ...
    fn all_model_presets() -> Vec<ModelPreset> {
        dcode_core::test_support::all_model_presets().clone()
    }
    ...
    async fn model_migration_prompt_skips_when_target_missing_or_hidden() {
        ...
        let mut current = available
            .iter()
            .find(|preset| preset.model == "gpt-5-dcode")
            .cloned()
        ...
        available.retain(|preset| preset.model != "gpt-5-dcode");
        ...
        let target = with_hidden_target
            .iter_mut()
            .find(|preset| preset.model == "gpt-5.2-dcode")
            .expect("target preset present");
        ...
        assert!(target_preset_for_upgrade(&with_hidden_target, "gpt-5.2-dcode").is_none());
    }

    #[tokio::test]
    async fn model_migration_prompt_shows_for_hidden_model() {
        let dcode_home = tempdir().expect("temp dcode home");
        ...
        let current = available_models
            .iter()
            .find(|preset| preset.model == "gpt-5.1-dcode")
            .cloned()
            .expect("gpt-5.1-dcode preset present");
        assert!(
            !current.show_in_picker,
            "expected gpt-5.1-dcode to be hidden from picker for this test"
        );
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder::new(&app.config.dcode_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    -> Result<()> {
        ...
        let dcode_home = tempdir()?;
        app.config.dcode_home = dcode_home.path().to_path_buf();
        std::fs::write(dcode_home.path().join("config.toml"), "[broken")?;
    ...
    async fn fresh_session_config_uses_current_service_tier() {
        ...
        assert_eq!(
            config.service_tier,
            Some(dcode_protocol::config_types::ServiceTier::Fast)
        );
    }
    ...
    async fn replayed_initial_messages_apply_rollback_in_queue_order() {
        ...
        app.handle_dcode_event_replay(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id,
                forked_from_id: None,
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("dcode resume my-session".to_string())
        );
    }
}
