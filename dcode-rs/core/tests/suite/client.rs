...
use dcode_core::ModelClient;
use dcode_core::ModelProviderInfo;
use dcode_core::NewThread;
use dcode_core::Prompt;
use dcode_core::ResponseEvent;
use dcode_core::ThreadManager;
use dcode_core::WireApi;
use dcode_core::auth::AuthCredentialsStoreMode;
use dcode_core::built_in_model_providers;
use dcode_core::default_client::originator;
use dcode_core::error::DcodeErr;
...
use dcode_protocol::config_types::ReasoningSummary;
use dcode_protocol::config_types::Settings;
use dcode_protocol::config_types::Verbosity;
use dcode_protocol::models::ContentItem;
use dcode_protocol::models::FunctionCallOutputContentItem;
use dcode_protocol::models::FunctionCallOutputPayload;
use dcode_protocol::models::ImageDetail;
use dcode_protocol::models::LocalShellAction;
use dcode_protocol::models::LocalShellExecAction;
use dcode_protocol::models::LocalShellStatus;
use dcode_protocol::models::MessagePhase;
use dcode_protocol::models::ReasoningItemContent;
use dcode_protocol::models::ReasoningItemReasoningSummary;
use dcode_protocol::models::ResponseItem;
use dcode_protocol::models::WebSearchAction;
...
use dcode_protocol::protocol::RolloutItem;
use dcode_protocol::protocol::RolloutLine;
use dcode_protocol::protocol::SessionMeta;
use dcode_protocol::protocol::SessionMetaLine;
...
use core_test_support::test_dcode::TestDcode;
use core_test_support::test_dcode::test_dcode;
...
/// Writes an `auth.json` into the provided `dcode_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        dcode_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = dcode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![dcode_protocol::models::ContentItem::InputText {
    ...
    let prior_system = dcode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![dcode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = dcode_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Dcode to resume from our file
    let dcode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_dcode()
        .with_home(dcode_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, dcode_home, session_path.clone())
        .await
        .expect("resume conversation");
    let dcode = test.dcode.clone();
    ...
    dcode
        ...
        .await
        .unwrap();
    wait_for_event(&dcode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let dcode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_dcode().with_model("gpt-5.1");
    let test = builder
        .resume(&server, dcode_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_dcode().with_auth(DcodeAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_dcode()
        .with_auth(DcodeAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/dcode", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_dcode()
        .with_auth(create_dummy_dcode_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/dcode/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let mut config = load_default_config_for_test(&dcode_home).await;
    config.model_provider = model_provider;

    let auth_manager =
        match DcodeAuth::from_auth_storage(dcode_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => dcode_core::test_support::auth_manager_from_auth(auth),
            Ok(None) => panic!("No DcodeAuth found in dcode_home"),
            Err(e) => panic!("Failed to load DcodeAuth: {e}"),
        };
    ...
    let NewThread { thread: dcode, .. } = thread_manager
        .start_thread(config)
        .await
        .expect("create new conversation");
...
async fn skills_append_to_developer_message() {
    ...
    let dcode_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = dcode_home.path().join("skills/demo");
    ...
    let _dcode_home_guard = dcode_home;
}
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestDcode { dcode, .. } = test_dcode().with_model("gpt-5.1").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let model_info =
        dcode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let conversation_id = ThreadId::new();
    let auth_manager =
        dcode_core::test_support::auth_manager_from_auth(DcodeAuth::from_api_key("Test API Key"));
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-dcode-primary-used-percent", "12.5")
        .insert_header("x-dcode-secondary-used-percent", "40.0")
        .insert_header("x-dcode-primary-window-minutes", "10")
        .insert_header("x-dcode-secondary-window-minutes", "60")
        .insert_header("x-dcode-primary-reset-at", "1704069000")
        .insert_header("x-dcode-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_dcode()
        .with_auth(DcodeAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&dcode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-dcode-max in tests → 95% usable context window
    ...
    wait_for_event(&dcode, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-dcode-primary-used-percent", "100.0")
        .insert_header("x-dcode-secondary-used-percent", "87.5")
        .insert_header("x-dcode-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-dcode-primary-window-minutes", "15")
        .insert_header("x-dcode-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_dcode();
    let dcode_fixture = builder.build(&server).await?;
    let dcode = dcode_fixture.dcode.clone();
    ...
    let token_event = wait_for_event(&dcode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&dcode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&dcode, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&dcode, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = DcodeErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_dcode_auth() -> DcodeAuth {
    DcodeAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Dcode sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
