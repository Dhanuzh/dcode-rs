...
use dcode_protocol::protocol::SubAgentSource;
...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let dcode_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-dcode-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_dcode().build(&server).await.expect("build test dcode");
    ...
    let initial_header = first_request
        .single_request()
        .header("x-dcode-turn-metadata")
        .expect("x-dcode-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-dcode-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-dcode-turn-metadata"
    );
    assert_eq!(
        initial_parsed
            .get("sandbox")
