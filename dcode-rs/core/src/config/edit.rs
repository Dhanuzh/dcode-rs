...
use dcode_protocol::config_types::TrustLevel;
...
pub async fn apply(
    ...
    let dcode_home = dcode_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&dcode_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(dcode_home: &Path) -> Self {
        Self {
            dcode_home: dcode_home.to_path_buf(),
            profile: None,
            edits: Vec::new(),
        }
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.dcode_home, self.profile.as_deref(), &self.edits)
    }
