...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, DcodeErr::ServerOverloaded));
}
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("dcode_other"),
    );
    headers.insert(
        "x-dcode-other-limit-name",
        http::HeaderValue::from_static("dcode_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let DcodeErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected DcodeErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let DcodeErr::UnexpectedStatus(err) = err else {
        panic!("expected DcodeErr::UnexpectedStatus, got {err:?}");
    };
