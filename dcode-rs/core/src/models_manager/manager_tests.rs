...
async fn get_model_info_tracks_fallback_usage() {
    let dcode_home = tempdir().expect("temp dir");
    let config = ConfigBuilder::default()
        .dcode_home(dcode_home.path().to_path_buf())
        .build()
        .await
        .expect("load default test config");
    let auth_manager = AuthManager::from_auth_for_testing(DcodeAuth::from_api_key("Test API Key"));
...
async fn refresh_available_models_sorts_by_priority() {
    ...
    let dcode_home = tempdir().expect("temp dir");
    let auth_manager =
        AuthManager::from_auth_for_testing(DcodeAuth::create_dummy_chatgpt_auth_for_testing());
...
fn models_request_telemetry_emits_auth_env_feedback_tags_on_failure() {
    ...
    let telemetry = ModelsRequestTelemetry {
        ...
        auth_env: crate::auth_env_telemetry::AuthEnvTelemetry {
            ...
            dcode_api_key_env_present: false,
            dcode_api_key_env_enabled: false,
    ...
    assert_eq!(
        tags.get("auth_env_dcode_api_key_present")
            .map(String::as_str),
        Some("false")
    );
    assert_eq!(
        tags.get("auth_env_dcode_api_key_enabled")
            .map(String::as_str),
        Some("false")
    );
    assert_eq!(
