...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "dcode_otel.log_only",
            tracing::Level::INFO,
            event.name = "dcode.api_request",
            ...
            auth.env_dcode_api_key_present = self.auth_env.dcode_api_key_env_present,
            auth.env_dcode_api_key_enabled = self.auth_env.dcode_api_key_env_enabled,
...
impl ModelsManager {
    ...
    ///
    /// Uses `dcode_home` to store cached model metadata and initializes with bundled catalog
    ...
    fn construct_model_info_from_candidates(
        ...
        // retry for namespaced slugs like `custom/gpt-5.3-dcode`.
    ...
    async fn refresh_available_models(&self, refresh_strategy: RefreshStrategy) -> CoreResult<()> {
        ...
        // For non-ChatGPT auth with a custom provider base URL (e.g. GitHub
        // Copilot, Anthropic, Groq), also try to fetch models from the
        // provider's /models endpoint so the /model picker shows relevant
        // models instead of the builtin OpenAI list.
        let has_custom_provider = self.provider.base_url.is_some();
        if self.auth_manager.auth_mode() != Some(AuthMode::Chatgpt) && !has_custom_provider {
            if matches!(
                refresh_strategy,
        ...
        // For custom providers (e.g. GitHub Copilot), fetch the standard
        // OpenAI-compatible models list and use it exclusively — do not merge
        // with the bundled OpenAI catalog (which has upgrade paths to models
        // that custom providers don't support).
        if has_custom_provider {
            return match refresh_strategy {
                ...
                RefreshStrategy::OnlineIfUncached => {
                    ...
                    self.fetch_custom_provider_models().await
                }
                RefreshStrategy::Online => self.fetch_custom_provider_models().await,
            };
        }
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            dcode_otel::start_global_timer("dcode.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth_manager.auth().await;
        let auth_mode = auth.as_ref().map(DcodeAuth::auth_mode);
        let api_provider = self.provider.to_api_provider(auth_mode)?;
        let api_auth = auth_provider_from_auth(auth.clone(), &self.provider)?;
        let auth_env = collect_auth_env_telemetry(
            &self.provider,
            self.auth_manager.dcode_api_key_env_enabled(),
        );
    ...
    /// Fetch models from a custom provider (e.g. GitHub Copilot, Groq) that
    /// returns the standard OpenAI-compatible `{"data": [{"id": "..."}]}` format.
    /// These models replace the active catalog entirely — we do NOT merge with
    /// the bundled `models.json` to avoid inheriting upgrade/migration paths
    /// that the custom provider doesn't support.
    async fn fetch_custom_provider_models(&self) -> CoreResult<()> {
        let auth = self.auth_manager.auth().await;
        let auth_mode = auth.as_ref().map(DcodeAuth::auth_mode);
        info!(
            provider = %self.provider.name,
            has_auth = auth.is_some(),
            "fetch_custom_provider_models: starting"
        );
        let api_provider = self.provider.to_api_provider(auth_mode)?;
        let api_auth = auth_provider_from_auth(auth.clone(), &self.provider)?;
        let transport = ReqwestTransport::new(build_reqwest_client());
        let client = dcode_api::endpoint::models::SimpleModelsClient::new(transport, api_provider, api_auth);

        let models = timeout(MODELS_REFRESH_TIMEOUT, client.list_models())
            .await
            .map_err(|_| {
                error!("fetch_custom_provider_models: timed out");
                DcodeErr::Timeout
            })?
            .map_err(|api_err| {
                error!("fetch_custom_provider_models: API error: {api_err:?}");
                map_api_error(api_err)
            })?;

        info!(count = models.len(), "fetch_custom_provider_models: fetched models");

        // Replace the catalog completely — no merge with the bundled models.
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            dcode_otel::start_global_timer("dcode.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::models_manager::client_version_to_whole();
