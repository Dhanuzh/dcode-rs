...
//!
//! `ModelClient` is intended to live for the lifetime of a Dcode session and holds the stable
...
//! per-turn state such as the `x-dcode-turn-state` token used for sticky routing.
//!
...
use dcode_api::CompactClient as ApiCompactClient;
use dcode_api::CompactionInput as ApiCompactionInput;
use dcode_api::MemoriesClient as ApiMemoriesClient;
use dcode_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use dcode_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use dcode_api::RawMemory as ApiRawMemory;
use dcode_api::RequestTelemetry;
use dcode_api::ReqwestTransport;
use dcode_api::ResponseCreateWsRequest;
use dcode_api::ResponsesApiRequest;
use dcode_api::ResponsesClient as ApiResponsesClient;
use dcode_api::ResponsesOptions as ApiResponsesOptions;
use dcode_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use dcode_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use dcode_api::SseTelemetry;
use dcode_api::TransportError;
use dcode_api::WebsocketTelemetry;
use dcode_api::build_conversation_headers;
use dcode_api::common::Reasoning;
use dcode_api::common::ResponsesWsRequest;
use dcode_api::create_text_param_for_request;
use dcode_api::error::ApiError;
use dcode_api::requests::responses::Compression;
use dcode_api::response_create_client_metadata;
use dcode_otel::SessionTelemetry;
use dcode_otel::current_span_w3c_trace_context;

use dcode_protocol::ThreadId;
use dcode_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use dcode_protocol::config_types::ServiceTier;
use dcode_protocol::config_types::Verbosity as VerbosityConfig;
...
use dcode_protocol::protocol::W3cTraceContext;
...
pub const X_CODEX_TURN_STATE_HEADER: &str = "x-dcode-turn-state";
pub const X_CODEX_TURN_METADATA_HEADER: &str = "x-dcode-turn-metadata";
...
struct CurrentClientSetup {
    auth: Option<DcodeAuth>,
    api_provider: dcode_api::Provider,
...
///
/// This holds configuration and state that should be shared across turns within a Dcode session
...
/// - The `x-dcode-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Dcode turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-dcode-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-dcode-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Dcode session. Per-turn values
    ...
    pub fn new(
        ...
        let dcode_api_key_env_enabled = auth_manager
            .as_ref()
            .is_some_and(|manager| manager.dcode_api_key_env_enabled());
        let auth_env_telemetry = collect_auth_env_telemetry(&provider, dcode_api_key_env_enabled);
        Self {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "dcode.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub async fn compact_conversation_history(
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(DcodeAuth::auth_mode),
    ...
    async fn current_client_setup(&self) -> Result<CurrentClientSetup> {
        ...
        let api_provider = self
            .state
            .provider
            .to_api_provider(auth.as_ref().map(DcodeAuth::auth_mode))?;
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&crate::auth::DcodeAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(DcodeAuth::is_chatgpt_auth)
    ...
    async fn stream_responses_api(
        ...
        if let Some(path) = &*CODEX_RS_SSE_FIXTURE {
            ...
            let stream = dcode_api::stream_from_fixture(
                path,
    ...
    async fn stream_chat_api(
        ...
        let chat_body = dcode_api::build_chat_request(
    ...
    /// Permanently disables WebSockets for this Dcode session and resets WebSocket state.
    ///
...
///
/// These headers implement Dcode-specific conventions:
///
/// - `x-dcode-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-dcode-turn-state`: sticky routing token captured earlier in the turn.
/// - `x-dcode-turn-metadata`: optional per-turn metadata for observability.
fn build_responses_headers(
    ...
    {
        headers.insert("x-dcode-beta-features", header_value);
    }
...
/// the mapped `DcodeErr` is returned to the caller.
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(DcodeErr::RefreshTokenFailed(failed))
            }
